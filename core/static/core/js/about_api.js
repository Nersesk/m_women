...
async function getAllPartners(page) {
  try {
    const response = await fetch(
      `https://տեստէջ.հայ/get_partners/${leng}/${page}`,
      {
        method: "GET",
        headers: {
...
async function getAllStaff(page) {
  try {
    const response = await fetch(
      `https://տեստէջ.հայ/get_staff/${leng}/${page}`,
      {
        method: "GET",
        headers: {
          Accept: "application/json",
...
async function appendStaff(staff_members, next_page) {
  ...
  staff_members.forEach((staff) => {
    ...
    img.setAttribute("src", `https://xn--29ae4dgic.xn--y9a3aq${staff.image}`);
...
function appendPartners(partners) {
  ...
  business_partners.forEach((partner) => {
    ...
    const img = document.createElement("img");
    img.setAttribute("src", `https://xn--29ae4dgic.xn--y9a3aq${partner.image}`);
