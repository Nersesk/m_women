...
async function getCount_list(page) {
  try {
    const response = await fetch(
      `https://տեստէջ.հայ/get_reports/${leng}/${page}`,
      {
        method: "GET",
        headers: {
...
async function appenCount_list(product_list, next_page) {
  ...
  product_list.forEach((product) => {
    ...
    img.setAttribute("src", `https://xn--29ae4dgic.xn--y9a3aq${product.image}`);
    p.innerHTML += `<span >${span_d}</span>`;
    img.setAttribute("alt", "images");
    div.classList.add("countcol");
    div.addEventListener("click", () => {
      let url = `https://xn--29ae4dgic.xn--y9a3aq${product.file}`;
