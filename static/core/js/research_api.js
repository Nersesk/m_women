...
async function getProduct_list(page) {
  try {
    const response = await fetch(
      `${server_url}product_list/${leng}/${page}`,
      {
        method: "GET",
        headers: {
...
async function appendProduct_list(product_list, next_page) {
  ...
  product_list.forEach((product) => {
    ...
    card_a.addEventListener("click", () => {
      ...
      tnt.innerText = lenguage[leng].videos
      photo.innerText = lenguage[leng].photo;
      file.innerText = lenguage[leng].files;
...
async function addModalTnt(url) {
  ...
  for (let index = 0; index < videos.length; index++) {
    ...
    iframe.setAttribute("src", videos[index].url);
